...
function compiledIntegrationPlan(phenotype, durationUs) {
  ...
  const localLeftUnits = [];
  const localRightUnits = [];
  ...
  for (const edge of edges) {
    ...
    if (weight === PROFILE.localEdgeWeightQ30) {
      localLeftUnits.push(left);
      localRightUnits.push(right);
    } else {
...
function integrateCompiledPlan(phases, amplitudes, sums, plan, iterations) {
  ...
  for (let step = 0; step < iterations; step += 1) {
    sums.fill(0);
    for (let edgeId = 0; edgeId < localEdgeCount; edgeId += 1) {
      const left = localLeftUnits[edgeId];
      const right = localRightUnits[edgeId];
    ...
    for (let unitId = 0; unitId < count; unitId += 1) {
      ...
      if (coupling !== 0) {
        ...
        if (response < -PROFILE.couplingResponseLimitQ30) {
          response = -PROFILE.couplingResponseLimitQ30;
        } else if (response > PROFILE.couplingResponseLimitQ30) {
          response = PROFILE.couplingResponseLimitQ30;
        }
      }
