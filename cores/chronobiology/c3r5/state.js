...
// Eight hours is 480 unchanged one-minute integration quanta. It keeps one
// mandatory --jitless transition inside the existing 20% CPU / 250 ms worker
// contract while retaining the exact frozen oscillator operation sequence.
const MAX_BOUNDED_INTEGRATION_SLICE_US = 8 * 3_600_000_000;
...
function normalizeState(input) {
  if (input && typeof input === 'object' && NORMALIZED_STATES.has(input)) {
    return input;
  }
  ...
  const normalized = freeze(candidate);
...
function advanceTrustedTime(state, event, { founderSeedHex } = {}) {
  ...
  const deferred = continuity.deferred_trusted_time_evidence;
  if (deferred && (evidence.runtime_revision < deferred.runtime_revision
    || (evidence.runtime_revision === deferred.runtime_revision
      && evidence.pulse_sequence < deferred.pulse_sequence))) {
    fail('bounded trusted-time catch-up rewound', 'CHRONOBIOLOGY_TIME_REWIND');
  }
  ...
  const maySlice = !maySkip
    && interval > MAX_BOUNDED_INTEGRATION_SLICE_US
    && continuity.photic_route_configured === false
    && continuity.pending_photic_evidence.length === 0;
  ...
  const integrationTargetUs = maySlice
    ? continuity.committed_through_us + MAX_BOUNDED_INTEGRATION_SLICE_US
    : evidence.trusted_time_us;
  const integrated = maySkip
    ...
    : integrateEvidencePlan(routedCurrent, integrationTargetUs);
  if (maySlice) {
    return normalizeState({
      ...routedCurrent,
      ...
      continuity: {
        ...continuity,
        committed_through_us: integrationTargetUs,
        ...
        pending_photic_evidence: integrated.pending_photic_evidence
          ?? continuity.pending_photic_evidence,
      },
    });
  }
...
module.exports = {
  MAX_BOUNDED_INTEGRATION_SLICE_US,
  TRUSTED_TIME_TOPIC,
  PHOTIC_TOPIC,
  advanceTrustedTime,
  bindState,
