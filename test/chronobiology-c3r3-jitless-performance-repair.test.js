...
const previous = require('../cores/chronobiology/c3r2');
const repaired = require('../cores/chronobiology/c3r3');
const { createFounderState } = require('../cores/chronobiology/c3r2/founder');
const referenceOscillator = require('../cores/chronobiology/c3r2/oscillator');
const repairedOscillator = require('../cores/chronobiology/c3r3/oscillator');
...
test('C3R3-ID-01 JITless repair is a new contained implementation identity', () => {
  const record = enforcePackagePolicy(require.resolve('../cores/chronobiology/c3r3'));
  assert.equal(verifyManifestAgainstPackagePolicy(record, repaired.manifest), true);
  assert.equal(previous.manifest.version, '1.0.0-c3rc.2');
  assert.equal(repaired.manifest.version, '1.0.0-c3rc.3');
  assert.equal(repaired.manifest.stage, 'c3-shadow-jitless-performance-repair');
...
test('C3R3-BIO-01 compiled engine is byte-identical across founders and remainder quanta', () => {
  const durations = [
    1,
    59_999_999,
    60_000_000,
  ...
  for (let vector = 0; vector < 8; vector += 1) {
    const state = founder(`C3RC.3 differential founder ${vector}`);
...
test('C3R3-ENTRY-01 real --jitless biological path preserves the golden state inside the contract', () => {
  const script = String.raw`
    const crypto = require('node:crypto');
    const { performance } = require('node:perf_hooks');
    const { stableStringify } = require('./runtime/kernel/canonical-json');
    const { emptyState, bindState, advanceTrustedTime } =
      require('./cores/chronobiology/c3r3/state');
    const binding = {
