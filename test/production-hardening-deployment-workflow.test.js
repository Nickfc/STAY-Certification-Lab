...
test('R111F deployment is manual-only and pins the merged Git and immutable archive identities', () => {
  ...
  assert.match(workflow, /RELEASE_TAG: r111f-v6/);
  assert.match(workflow, /RELEASE_COMMIT: 589a711abac971889993fde124b5e263f6400505/);
  assert.match(workflow, /RELEASE_TREE: c95ff9e2db4f99f9ec35e28f3c5a8dc336abef86/);
  assert.match(workflow, /ARCHIVE_SHA256: f1e668a63caca869c212a0e4d178d8e9829985fb29e77869a67a74b84c5634e3/);
...
test('R111F deployment validates the archive, clean extraction, Git payload, syntax and focused regressions before secrets', () => {
  ...
  assert.match(validation, /\$\{#entries\[@\]\} == 86/);
...
test('R111F mutation requires exact authorization, bounded root execution and forward-only recovery', () => {
  assert.match(workflow, /AUTHORIZE_R111F_V6_FORWARD_WITH_FENCED_RECOVERY/);
  assert.match(workflow, /AUTHORIZE_R111F_V6_FORWARD_RECOVERY_ONLY/);
...
test('R111F completion proves the frozen revision and active benchmark before bounded cleanup', () => {
  ...
  assert.match(workflow, /\^\/opt\/stay\/incoming\/r111f-v6-\[0-9\]\+\$/);
