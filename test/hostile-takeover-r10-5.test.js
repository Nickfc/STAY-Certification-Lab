...
test('R10.5-08 production unit requires OS sandbox, signed promotion, mandatory policy and setup-only capability bound', () => {
  ...
  assert.match(service, /^CapabilityBoundingSet=CAP_SETGID CAP_SETUID CAP_NET_ADMIN CAP_SYS_CHROOT CAP_SYS_PTRACE CAP_SYS_ADMIN$/m); assert.match(service, /^AmbientCapabilities=$/m); assert.match(service, /^ReadWritePaths=\/var\/lib\/stay\/data$/m);
});
