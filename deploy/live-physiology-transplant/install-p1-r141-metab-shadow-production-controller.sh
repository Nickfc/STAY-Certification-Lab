...
EXPECTED_WRAPPER_SHA256='18907d1b032744cd28d5488635d1d174b335835d6a744a0068942d19181e6e2a'
...
[[ "$EUID" -eq 0 ]] || {
  echo 'R141_BOOTSTRAP_ABORT=root-required' >&2
  exit 60
}
...
[[ "$observed_private_ipv4" == "$EXPECTED_PRIVATE_IPV4" ]] || {
  echo 'R141_BOOTSTRAP_ABORT=host-identity-mismatch' >&2
  exit 61
}
[[ -f "$SOURCE_WRAPPER" && ! -L "$SOURCE_WRAPPER" ]] || {
  echo 'R141_BOOTSTRAP_ABORT=wrapper-source-invalid' >&2
  exit 62
}
[[ "$(sha256sum "$SOURCE_WRAPPER" | awk '{print $1}')" == "$EXPECTED_WRAPPER_SHA256" ]] || {
  echo 'R141_BOOTSTRAP_ABORT=wrapper-source-hash-mismatch' >&2
  exit 63
}
command -v visudo >/dev/null 2>&1 || {
  echo 'R141_BOOTSTRAP_ABORT=visudo-unavailable' >&2
  exit 64
}
id staydeploy >/dev/null 2>&1 || {
  echo 'R141_BOOTSTRAP_ABORT=staydeploy-user-missing' >&2
  exit 65
}

staging="$(mktemp -d /run/stay-r141-metab-shadow-recovery-v2-bootstrap.XXXXXX)"
cleanup() {
  local status=$?
  trap - EXIT
  [[ "$staging" =~ ^/run/stay-r141-metab-shadow-recovery-v2-bootstrap\.[A-Za-z0-9]+$ &&
...
echo 'R141_FORWARD_AUTHORIZED=NO'
echo 'R141_RECOVERY_AUTHORIZED=NO'
