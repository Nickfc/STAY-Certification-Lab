...
const BASELINE = Object.freeze({
  ...
  checkpointId: '039d2b99-b950-4bab-beac-83f602b27be2',
  checkpointGeneration: 5118,
  ...
  consumerCursor: 2341576,
  failedSequence: 2341577,
});
...
const REPAIR = Object.freeze({
  ...
  releaseManifestHash: 'sha256:3de334f5360ad269c9e757a606c6815d3308550cc3af01b3bb908f8ff3d8790a',
  ...
  checkpointGeneration: 5119,
  checkpointId: 'chronobiology-c3r5-repair-f1e1ae54-5119',
  repairId: 'chronobiology-c3r5-r118-bounded-catchup',
});

function fail(message, code = 'R119F_CHRONOBIOLOGY_REPAIR') {
...
function validateReleaseIdentity(releaseRoot) {
  ...
  assert(`sha256:${sha256(fs.readFileSync(historicalEntrypoint))}` === BASELINE.moduleHash,
    'historical Chronobiology module identity changed', 'R119F_REPAIR_RELEASE_IDENTITY');
  assert(canonicalHash(historicalManifest) === BASELINE.releaseManifestHash,
    'historical Chronobiology manifest identity changed', 'R119F_REPAIR_RELEASE_IDENTITY');
  assert(canonicalHash(historicalDurableManifest) === BASELINE.manifestHash,
    ...
    'R119F_REPAIR_RELEASE_IDENTITY');
  assert(historical.policy.policyHash === BASELINE.packagePolicyHash,
    'historical Chronobiology package policy changed', 'R119F_REPAIR_RELEASE_IDENTITY');
  assert(`sha256:${sha256(fs.readFileSync(repairEntrypoint))}` === REPAIR.moduleHash,
    'repaired Chronobiology module identity changed', 'R119F_REPAIR_RELEASE_IDENTITY');
  assert(canonicalHash(repairedManifest) === REPAIR.releaseManifestHash,
    'repaired Chronobiology manifest identity changed', 'R119F_REPAIR_RELEASE_IDENTITY');
  ...
  assert(repaired.policy.policyHash === REPAIR.packagePolicyHash,
    'repaired Chronobiology package policy changed', 'R119F_REPAIR_RELEASE_IDENTITY');
  ...
  'repaired Chronobiology manifest is not contained', 'R119F_REPAIR_RELEASE_IDENTITY');
  assert(stableStringify(repairedManifest.resources) === stableStringify(historicalManifest.resources),
    'Chronobiology resource contract changed', 'R119F_REPAIR_RESOURCE_CONTRACT');
  ...
  'Chronobiology signalling contract changed', 'R119F_REPAIR_SIGNALLING_CONTRACT');
...
function metadataRevision(database) {
  ...
  assert(row && sha256(row.json) === row.sha256,
    'runtime revision metadata integrity failed', 'R119F_REPAIR_RUNTIME_REVISION');
  const revision = Number(JSON.parse(row.json).revision);
  assert(Number.isSafeInteger(revision),
    'runtime revision is invalid', 'R119F_REPAIR_RUNTIME_REVISION');
  return revision;
}
...
function assertQuiescentBaseline(database, { allowRepairedIdentity = false } = {}) {
  assert(database.prepare('PRAGMA quick_check').get()?.quick_check === 'ok',
    'SQLite quick-check failed', 'R119F_REPAIR_DATABASE_INTEGRITY');
  assert(metadataRevision(database) === BASELINE.runtimeRevision,
    'durable runtime revision is not the exact R118 recovery boundary',
    'R119F_REPAIR_RUNTIME_REVISION');
  ...
  'Chronobiology source checkpoint identity changed', 'R119F_REPAIR_CHECKPOINT_FENCE');
  ...
  if (allowRepairedIdentity) {
    ...
    'Chronobiology repair checkpoint identity changed', 'R119F_REPAIR_CHECKPOINT_FENCE');
  }
  ...
  'Chronobiology consumer is not quiescent', 'R119F_REPAIR_CONSUMER_FENCE');
  ...
  'Chronobiology has pending delivery debt', 'R119F_REPAIR_PENDING_DEBT');
  assert(scalar(database,
    "SELECT COUNT(*) value FROM biological_outbox_intents WHERE producer_core_id=? AND status='PENDING'",
    BASELINE.coreId) === 0,
  'Chronobiology has pending output debt', 'R119F_REPAIR_OUTPUT_DEBT');
  assert(scalar(database,
    'SELECT COUNT(*) value FROM authority WHERE core_id=?', BASELINE.coreId) === 0,
  'Chronobiology owns authority', 'R119F_REPAIR_AUTHORITY');
  assert(scalar(database,
    "SELECT COUNT(*) value FROM biological_outbox_intents WHERE producer_core_id=?",
    'sntss') === 0,
  'SNTSS zero-output contract changed', 'R119F_REPAIR_SNTSS_OUTPUT');
...
function checkpointBlob(databasePath, checkpoint) {
  ...
  'Chronobiology checkpoint blob integrity failed', 'R119F_REPAIR_CHECKPOINT_BLOB');
  return bytes;
}
...
function applyRepair({
  ...
  try {
    ...
    assert(manifests.repairedManifest.stateSchema === Number(snapshot.sourceCheckpoint.state_schema),
      'Chronobiology checkpoint schema changed', 'R119F_REPAIR_CHECKPOINT_STATE');
...
function rollbackRepair({ databasePath, releaseRoot, now = () => new Date().toISOString() }) {
  ...
  try {
    ...
    'Chronobiology advanced beyond the rollback fence', 'R119F_REPAIR_ROLLBACK_FENCE');
    ...
    assert(updated.changes === 1, 'Chronobiology repair rollback lost its fence',
      'R119F_REPAIR_ROLLBACK_COMPARE_AND_SWAP');
...
function parseArguments(argv) {
  ...
  assert(['preflight', 'apply', 'rollback'].includes(mode) && databasePath && releaseRoot,
    'usage: p1-r119f-chronobiology-bounded-catchup-repair.js preflight|apply|rollback DATABASE RELEASE_ROOT',
    'R119F_REPAIR_ARGUMENTS');
...
if (require.main === module) {
  try {
    ...
    process.stderr.write(`R119F_CHRONOBIOLOGY_REPAIR_ABORT=${error.code || 'ERROR'}:${error.message}\n`);
    process.exitCode = 1;
  }
}
