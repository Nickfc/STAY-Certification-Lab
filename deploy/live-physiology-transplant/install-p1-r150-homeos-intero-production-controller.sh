...
EXPECTED_WRAPPER_SHA256='baeeb31a23cdacde94871ff40aba8423ace90bedd4f323ea15edf9d20968683d'
...
staging="$(mktemp -d /run/stay-r150-controller-v18-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v18-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
