...
EXPECTED_WRAPPER_SHA256='7d8a486483c797c2dfecfcc485c3909c5b02a9b43bc0abf85f16951505ffefe9'
...
staging="$(mktemp -d /run/stay-r150-controller-v28-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v28-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
...
printf '%s\n' 'P1_PRIVILEGED_BRIDGE_BOOTSTRAP=PASS' 'HOST_IDENTITY_GUARD=PASS' \
  ...
  'R148_HOMEOS_CAPACITY_SOURCE_FINALIZATION_AUTHORIZED=NO' \
