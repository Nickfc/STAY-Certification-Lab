...
EXPECTED_WRAPPER_SHA256='50f662cefce8658cf3f2c7b7572cdb3195aa892c3c05bb67378273c5093e4760'
...
staging="$(mktemp -d /run/stay-r150-controller-v22-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v22-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
