...
EXPECTED_WRAPPER_SHA256='f70cb15c890d396ba6013e3747e464d8a297bd3add5eb958b9c3d693a1a6d404'
...
staging="$(mktemp -d /run/stay-r150-controller-v4-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v4-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
