...
EXPECTED_WRAPPER_SHA256='65e6c81f8784759741000661e774963f8a214e28593beac5c8797b5965ddafd4'
...
staging="$(mktemp -d /run/stay-r150-controller-v1-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v1-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
