...
EXPECTED_WRAPPER_SHA256='951b234547cb5be522a73f105c219336ed0ee6cff51ddd92e56b821d55eee011'
...
staging="$(mktemp -d /run/stay-r150-controller-v3-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v3-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
