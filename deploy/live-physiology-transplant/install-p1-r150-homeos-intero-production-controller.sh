...
EXPECTED_WRAPPER_SHA256='b3e5c5aa2aa87cf5fdd38b001181875ce229191c13b1cd48a6e686759e719464'
...
staging="$(mktemp -d /run/stay-r150-controller-v19-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v19-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
