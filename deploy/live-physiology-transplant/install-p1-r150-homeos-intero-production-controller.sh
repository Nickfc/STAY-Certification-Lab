...
EXPECTED_WRAPPER_SHA256='753a048fe7879f662c5e5d66d79d008223d1ac55c7386c79bbfe5da607a66433'
...
staging="$(mktemp -d /run/stay-r150-controller-v8-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v8-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
