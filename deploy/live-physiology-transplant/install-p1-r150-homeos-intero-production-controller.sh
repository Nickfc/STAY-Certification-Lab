...
EXPECTED_WRAPPER_SHA256='de0bab3adc6f0cc37e10b7ac420570017b18959fac3f1bad40f7d6afbebff753'
...
staging="$(mktemp -d /run/stay-r150-controller-v12-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v12-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
