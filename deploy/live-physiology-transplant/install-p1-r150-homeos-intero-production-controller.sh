...
EXPECTED_WRAPPER_SHA256='562a5a60f193d182e95f983f587e085aaac1a08deb43f55668dc8f34af74244d'
...
staging="$(mktemp -d /run/stay-r150-controller-v2-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v2-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
