...
EXPECTED_WRAPPER_SHA256='760927cc803b7319e2cb07312ed49d6f9aa66ec8758235bb9c038808ac65af67'
...
staging="$(mktemp -d /run/stay-r150-controller-v10-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v10-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
