...
EXPECTED_WRAPPER_SHA256='1ac4e43c314dc9b45cc6711888ac00af5da4abb80357782ddf9915bdb1ca11a3'
...
staging="$(mktemp -d /run/stay-r150-controller-v9-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v9-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
