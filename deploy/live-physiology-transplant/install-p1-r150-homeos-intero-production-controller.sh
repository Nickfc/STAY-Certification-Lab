...
EXPECTED_WRAPPER_SHA256='52223d9f1d0d5ebc184e12fdfa365dac9ac3b3ba782e02e173e589a7fd333f14'
...
staging="$(mktemp -d /run/stay-r150-controller-v29-bootstrap.XXXXXX)"
cleanup(){ local status=$?;trap - EXIT;[[ "$staging" =~ ^/run/stay-r150-controller-v29-bootstrap\.[A-Za-z0-9]+$ && -d "$staging" && ! -L "$staging" ]]||exit 66;rm -rf --one-file-system -- "$staging";exit "$status"; }
trap cleanup EXIT
...
printf '%s\n' 'P1_PRIVILEGED_BRIDGE_BOOTSTRAP=PASS' 'HOST_IDENTITY_GUARD=PASS' \
  ...
  'R148_HOMEOS_SNTSS_RESTART_ANCHOR_RECOVERY_AUTHORIZED=NO' \
