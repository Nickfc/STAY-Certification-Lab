...
const REASON = 'r148-homeos-init-forward-preflight-v1';

function fail(message) {
  throw Object.assign(new Error(message), {
    code: 'P1_R148_INIT_SNAPSHOT_CREATE'
  });
}
...
async function main() {
  ...
  try {
    ...
    process.stdout.write(JSON.stringify({
      format: 'stay-r148-init-forward-preflight-snapshot-v1',
      result: 'PASS',
      name: snapshot.name,
      path: snapshot.path,
      manifestSha256:
...
main().catch(error => {
  console.error(`${error.code || 'P1_R148_INIT_SNAPSHOT_CREATE'}: ${error.message}`);
  process.exitCode = 1;
});
