...
: "${STAY_R119F_WORK:?}"
: "${STAY_R119F_BEFORE_DATABASE:?}"
: "${STAY_R119F_SERVICE_PROOF:?}"
: "${STAY_R119F_ENTRY_PROOF:?}"
: "${STAY_R119F_PREFLIGHT_PROOF:?}"
: "${STAY_R119F_RELEASE:?}"
...
: "${STAY_R119F_PRIVATE_IPV4:?}"

WORK="$(readlink -f -- "$STAY_R119F_WORK")"
BEFORE_DATABASE="$(readlink -f -- "$STAY_R119F_BEFORE_DATABASE")"
SERVICE_PROOF="$(readlink -f -- "$STAY_R119F_SERVICE_PROOF")"
ENTRY_PROOF="$(readlink -f -- "$STAY_R119F_ENTRY_PROOF")"
PREFLIGHT_PROOF="$(readlink -f -- "$STAY_R119F_PREFLIGHT_PROOF")"
NEW_RELEASE="$(readlink -f -- "$STAY_R119F_RELEASE")"
...
BENCHMARK_ROOT='/var/lib/stay/evidence/physiology-benchmark/R119F'
TARGET_FREEZE='/var/lib/stay/evidence/runtime-freezes/R119.json'
SCRIPT_DIRECTORY="$(dirname -- "$(readlink -f -- "$0")")"
LIVE_PROOF="$SCRIPT_DIRECTORY/p1-r119f-live-proof.js"
FREEZE_HELPER="$SCRIPT_DIRECTORY/p1-r119f-freeze.js"
...
install_atomic() {
  ...
  temporary="$(mktemp "$(dirname "$target")/.r119f-finalize.XXXXXX")"
...
cleanup() {
  ...
  if [[ "$COMPLETED" -eq 0 ]]; then
    ...
    if [[ "$BENCHMARK_ROOT_CREATED" -eq 1 && "$BENCHMARK_ROOT" == /var/lib/stay/evidence/physiology-benchmark/R119F && -d "$BENCHMARK_ROOT" && ! -L "$BENCHMARK_ROOT" ]]; then
      rm -rf --one-file-system -- "$BENCHMARK_ROOT"
    fi
    if [[ "$FREEZE_CREATED" -eq 1 && "$TARGET_FREEZE" == /var/lib/stay/evidence/runtime-freezes/R119.json && -f "$TARGET_FREEZE" && ! -L "$TARGET_FREEZE" ]]; then
      rm -f -- "$TARGET_FREEZE"
    fi
  fi
  exit "$status"
...
[[ "$WORK" == /var/lib/stay/evidence/production-hardening/.R119F-* && -d "$WORK" && ! -L "$WORK" ]] ||
...
[[ "$NEW_RELEASE" == /opt/stay/releases/0.8.11.3-p1m-r119f-chrono-repair-* \
...
for step in $(seq 1 26); do
  sleep 5
  if (( step % 2 == 0 )); then
    echo "R119F_LIVE_GATE_PROGRESS_SECONDS=$((step * 5))"
  fi
done
...
for _ in $(seq 1 60); do
  ...
  if [[ "$(json_field "$sntss" resident.running 2>/dev/null || true)" == true \
    ...
    && "$(json_field "$meta" revision 2>/dev/null || true)" == 119 ]]; then break; fi
  sleep 1
done
...
node "$FREEZE_HELPER" capture \
  ...
  --release "$NEW_RELEASE" \
  --release-tag "$STAY_R119F_RELEASE_TAG" \
  --release-commit "$STAY_R119F_RELEASE_COMMIT" \
  --release-tree "$STAY_R119F_RELEASE_TREE" \
  --archive-sha256 "$STAY_R119F_ARCHIVE_SHA256" \
  --manifest-sha256 "$STAY_R119F_MANIFEST_SHA256" \
  --controller-sha256 "$STAY_R119F_CONTROLLER_SHA256" \
  --hostname "$(hostname)" \
  --private-ip "$STAY_R119F_PRIVATE_IPV4" > "$WORK/freeze.json" ||
...
temporary_freeze="$(mktemp "$(dirname "$TARGET_FREEZE")/.R119.XXXXXX")"
...
for _ in $(seq 1 30); do
  ...
  if [[ "$(json_field "$final_meta" revisionLabel 2>/dev/null || true)" == R119F ]]; then break; fi
  sleep 1
done
[[ "$(json_field "$final_meta" revision 2>/dev/null || true)" == 119 \
  && "$(json_field "$final_meta" revisionFrozen 2>/dev/null || true)" == true \
  && "$(json_field "$final_meta" revisionLabel 2>/dev/null || true)" == R119F ]] ||
...
[Unit]
Description=STAY R119F BSF SNTSS Chronobiology performance-repair 72-hour benchmark
After=stay.service
...
R119F_FINALIZE_RESULT=PASS
RUNTIME_REVISION_BEFORE=118
COLD_RECOVERY_REVISION=119
RUNTIME_REVISION_AFTER=119
REVISION_LABEL=R119F
CURRENT_RELEASE=$NEW_RELEASE
SERVICE_RESTARTS_THIS_DEPLOYMENT=ONE
CHRONOBIOLOGY_VERSION=1.0.0-c3rc.5
