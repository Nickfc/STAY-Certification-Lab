...
const { BASELINE, REPAIR } = require('./p1-r118f-chronobiology-implementation-repair');
...
function fail(message, code = 'R118F_LIVE_PROOF') {
...
function readJson(file, label) {
  ...
  catch (error) { fail(`${label} is invalid: ${error.message}`, 'R118F_LIVE_PROOF_INPUT'); }
}
...
function validateBefore(before) {
  ...
  'SNTSS before identity changed', 'R118F_BEFORE_SNTSS');
  ...
  'Chronobiology before identity changed', 'R118F_BEFORE_CHRONOBIOLOGY');
  ...
  assert(before.chronobiologyPendingDeliveries === 0
    ...
    && before.latestImplementationRepair === null,
  'before snapshot is not authority/output/debt contained', 'R118F_BEFORE_CONTAINMENT');
  ...
  'completed 4,096-event cold replay evidence changed', 'R118F_BEFORE_COLD_REPLAY');
  return { sntss, chrono };
}
...
function validateStatus(status, expected) {
  ...
  `${expected.label} resource limits changed`, 'R118F_RESOURCE_CONTRACT');
  return value;
}
...
function validateMeta(meta) {
  ...
  assert(meta.ok === true && meta.revision === 118,
    'public metadata is not R118', 'R118F_PUBLIC_META');
  assert(bsf?.mode === 'LIVE' && bsf?.status === 'RUNNING'
    && bsf?.running === true && bsf?.healthOk === true
    && Number(bsf?.writeFailures) === 0,
  'BSF is not LIVE and healthy', 'R118F_BSF');
  ...
  'fetus continuity or memory contract changed', 'R118F_FETUS');
  ...
  'web-chip acceptance state is invalid', 'R118F_CHIPS');
  return { bsf, fetus };
}
...
function verify({ before, after, sntssStatus, chronobiologyStatus, meta, service }) {
  ...
  'after snapshot is not durable R118', 'R118F_AFTER_REVISION');
  ...
  'SNTSS lineage or progression changed', 'R118F_AFTER_SNTSS');
  assert(chrono?.instance_id === BASELINE.instanceId
  ...
  'Chronobiology repair identity is not running', 'R118F_AFTER_CHRONOBIOLOGY');
  assert(chronoConsumer
    && Number(chronoConsumer.required) === 0
  ...
  'after snapshot has debt, output, or authority leakage', 'R118F_AFTER_CONTAINMENT');
  ...
  'implementation repair evidence is invalid', 'R118F_REPAIR_EVIDENCE');
  assert(biologicalResync?.runtimeRevision === 117
    && biologicalResync?.abandonedCount === 0
    && biologicalResync?.inventedBiologicalTime === false
    && residentResync?.runtimeRevision === 117
    && residentResync?.abandonedCount === 0
    && residentResync?.inventedBiologicalTime === false,
  'R117 recovery invented or abandoned biological history', 'R118F_RECOVERY_EVIDENCE');
  ...
  'new resident fault evidence appeared after repair', 'R118F_NEW_FAULT');
  ...
  'service restart fence is invalid', 'R118F_SERVICE_RESTART');

  return Object.freeze({
    format: 'stay-r118f-live-proof-v1',
    result: 'PASS',
    runtime: { fromRevision: 116, recoveryRevision: 117, toRevision: 118 },
    service,
    release: {
...
function main(argv = process.argv.slice(2)) {
  ...
  fail('capture or verify inputs required', 'R118F_LIVE_PROOF_USAGE');
}

if (require.main === module) {
  try { main(); }
  catch (error) {
    process.stderr.write(`R118F_LIVE_PROOF_ABORT=${error.code || 'FAILED'}:${error.message}\n`);
    process.exitCode = 1;
  }
}
