...
TARGET_FREEZE="$FREEZE_DIR/R137.json"
...
DROPIN="$DROPIN_DIR/r137-metab-shadow-recovery-once.conf"
...
AUTHORIZATION='AUTHORIZE_R137_METAB_OUTPUT_FIREWALLED_SHADOW_FORWARD_RECOVERY_ONLY'
PROMOTION_AUTHORIZATION='AUTHORIZE_R137_METAB_NEUTRAL_TO_OUTPUT_FIREWALLED_SHADOW_RECOVERY_ONLY'

: "${STAY_R137_RECOVERY_AUTHORIZATION:?}"
: "${STAY_R137_TARGET_RELEASE:?}"
: "${STAY_R137_RELEASE_TAG:?}"
: "${STAY_R137_RELEASE_COMMIT:?}"
: "${STAY_R137_RELEASE_TREE:?}"
: "${STAY_R137_ARCHIVE_SHA256:?}"
: "${STAY_R137_MANIFEST_SHA256:?}"
: "${STAY_R137_CONTROLLER_SHA256:?}"
...
abort() { printf 'R137_METAB_SHADOW_RECOVERY_ABORT=%s\n' "$1" >&2; exit "${2:-1}"; }
...
point_current() {
  local target="$1" temporary="/opt/stay/.current-r137-$$"
...
install_atomic() {
  ...
  temporary="$(mktemp "$(dirname "$target")/.r137-metab.XXXXXX")"
...
capture_quiescent() {
  ...
  for attempt in $(seq 1 20); do
    /usr/local/bin/node "$STAY_R137_TARGET_RELEASE/$PROOF" capture "$DATABASE" > "$temporary"
    if /usr/local/bin/node - "$temporary" <<'NODE'
'use strict';const v=JSON.parse(require('node:fs').readFileSync(process.argv[2],'utf8'));
const settled=v.runtimeRevision===135||(v.capacitySource?.pending===null&&
...
cleanup() {
  ...
  if [[ "$COMPLETED" -eq 0 ]]; then
    ...
    if [[ "$RESTART_COMMITTED" -eq 0 && "$POINTER_SWITCHED" -eq 1 &&
      "$(readlink -f /opt/stay/current 2>/dev/null)" == "$STAY_R137_TARGET_RELEASE" ]]; then
      point_current "$SOURCE_RELEASE"
    fi
    if [[ -n "$WORK" && -d "$WORK" ]]; then
      failed="$(mktemp -d "$EVIDENCE_ROOT/FAILED-R137-METAB-SHADOW-$(date -u +'%Y%m%dT%H%M%SZ').XXXXXX")"
      rmdir -- "$failed"; mv -T "$WORK" "$failed"; WORK=''; chmod -R a-w "$failed"
      printf 'R137_METAB_SHADOW_FAILURE_EVIDENCE=%s\n' "$failed" >&2
    fi
    if [[ "$RESTART_COMMITTED" -eq 1 ]]; then
      marker_tmp="$(mktemp /run/.stay-r137-metab-shadow-recovery.XXXXXX)"
      printf '%s\n' \
        "R137_FAILURE_EVIDENCE=$failed" \
        "R137_SOURCE_RELEASE=$SOURCE_RELEASE" \
        "R137_TARGET_RELEASE=$STAY_R137_TARGET_RELEASE" \
        "R137_RELEASE_TAG=$STAY_R137_RELEASE_TAG" \
        "R137_RELEASE_COMMIT=$STAY_R137_RELEASE_COMMIT" \
        "R137_RELEASE_TREE=$STAY_R137_RELEASE_TREE" \
        "R137_ARCHIVE_SHA256=$STAY_R137_ARCHIVE_SHA256" \
        "R137_MANIFEST_SHA256=$STAY_R137_MANIFEST_SHA256" \
        "R137_CONTROLLER_SHA256=$STAY_R137_CONTROLLER_SHA256" > "$marker_tmp"
      install_atomic "$marker_tmp" "$R137_MARKER" 0400
      rm -f -- "$marker_tmp"
      printf 'R137_METAB_SHADOW_FORWARD_RECOVERY_REQUIRED=YES\n' >&2
    fi
  fi
  exit "$status"
}
...
[[ "$STAY_R137_RECOVERY_AUTHORIZATION" == "$AUTHORIZATION" ]] || abort authorization-required 3302
[[ "$(ip -4 -o addr show scope global | awk '{split($4,a,"/");print a[1]}' | sort -u)" == "$EXPECTED_PRIVATE_IPV4" ]] || abort host-identity-mismatch 3303
[[ "$STAY_R137_TARGET_RELEASE" =~ ^/opt/stay/releases/0\.8\.11\.3-p1m-r137-metab-shadow-recovery-[0-9a-f]{12}$ &&
  "$STAY_R137_RELEASE_TAG" =~ ^r137-metab-shadow-recovery-v[0-9]+$ &&
  "$STAY_R137_RELEASE_COMMIT" =~ ^[0-9a-f]{40}$ && "$STAY_R137_RELEASE_TREE" =~ ^[0-9a-f]{40}$ &&
  "$STAY_R137_ARCHIVE_SHA256" =~ ^sha256:[0-9a-f]{64}$ &&
  "$STAY_R137_MANIFEST_SHA256" =~ ^sha256:[0-9a-f]{64}$ &&
  "$STAY_R137_CONTROLLER_SHA256" =~ ^sha256:[0-9a-f]{64}$ ]] || abort immutable-identity-invalid 3304
for directory in "$SOURCE_RELEASE" "$STAY_R137_TARGET_RELEASE" "$EVIDENCE_ROOT" "$FREEZE_DIR"; do
  [[ -d "$directory" && ! -L "$directory" ]] || abort release-or-evidence-root-invalid 3305
done
for file in "$DATABASE" "$PARENT_FREEZE" "$SOURCE_MARKER" \
  "$SOURCE_RELEASE/$SOURCE_MANIFEST" "$STAY_R137_TARGET_RELEASE/$PROOF" \
  "$STAY_R137_TARGET_RELEASE/$CLIENT" "$STAY_R137_TARGET_RELEASE/P1_R137_RELEASE.env"; do
...
[[ "$(readlink -f /opt/stay/current)" == "$SOURCE_RELEASE" &&
  ...
  "$(durable_runtime_revision)" == 135 && ! -e "$TARGET_FREEZE" && ! -L "$TARGET_FREEZE" &&
  ! -e "$R137_MARKER" && ! -L "$R137_MARKER" ]] ||
  abort r135-source-boundary-invalid 3307
for revision in 128 129 130 131 132 133 134 135 136; do
...
[[ "$(stat -Lc '%U:%G:%a' "$SOURCE_MARKER")" == 'root:root:400' &&
  "$(wc -l < "$SOURCE_MARKER")" -eq 9 ]] || abort r135-marker-trust-invalid 3309
for exact in \
  'R135_TARGET_RELEASE=/opt/stay/releases/0.8.11.3-p1m-r135-metab-shadow-recovery-0d123d94a809' \
  'R135_RELEASE_COMMIT=4db441f00c687c1e638ff18aa01af289a1841bff' \
  'R135_MANIFEST_SHA256=sha256:0d123d94a809600922e96802be8012a0700260a0f6a5497c51f36c125af528ee' \
  'R135_CONTROLLER_SHA256=sha256:0cbc82d406e49447e39971dcabf04c7c7e10ce577949c58fe3ca5ab2d7c53f0c'; do
  grep -Fx "$exact" "$SOURCE_MARKER" >/dev/null || abort r135-marker-cohort-invalid 3310
done
r135_failure="$(sed -n 's/^R135_FAILURE_EVIDENCE=//p' "$SOURCE_MARKER")"
[[ "$(grep -c '^R135_FAILURE_EVIDENCE=' "$SOURCE_MARKER")" -eq 1 &&
  "$r135_failure" =~ ^/var/lib/stay/evidence/production-hardening/FAILED-R135-METAB-SHADOW-[A-Za-z0-9TZ.-]+$ &&
  -d "$r135_failure" && ! -L "$r135_failure" && "$(stat -Lc '%U:%G' "$r135_failure")" == root:root &&
  -z "$(find -P "$r135_failure" -xdev \( -type l -o ! -type d ! -type f -o ! -user root -o ! -group root -o -perm /022 \) -print -quit)" ]] ||
  abort r135-failure-evidence-invalid 3311
...
WORK="$(mktemp -d "$EVIDENCE_ROOT/.R137-METAB-SHADOW-$(date -u +'%Y%m%dT%H%M%SZ').XXXXXX")"
install -o root -g root -m 0400 "$PARENT_FREEZE" "$WORK/R127.freeze.json"
install -o root -g root -m 0400 "$SOURCE_MARKER" "$WORK/R135.failure-marker.env"
...
if(!(current==='/opt/stay/releases/0.8.11.3-p1m-r135-metab-shadow-recovery-0d123d94a809'&&db.quickCheck==='ok'&&db.runtimeRevision===135&&
...
process.stdout.write(JSON.stringify({result:'PASS',runtimeRevision:135,sntssCheckpointGeneration:Number(row('resident:sntss').checkpoint_generation),chronobiologyCheckpointGeneration:Number(row('resident:chronobiology').checkpoint_generation),fetusInstanceId:fetusAuthority.instance_id,metabInstanceId:row('resident:metab').instance_id,metabCheckpointHash:row('resident:metab').checkpoint_hash,abandonedDeliveries:db.abandonedDeliveries,founder:db.metabCheckpointState.founder,metabChipHistory:db.metabChipHistory})+'\n');
NODE

install -d -o root -g root -m 0755 "$DROPIN_DIR"
dropin_tmp="$(mktemp /run/stay-r137-metab-shadow-recovery-once.XXXXXX)"
printf '%s\n' '[Service]' \
...
point_current "$STAY_R137_TARGET_RELEASE"; POINTER_SWITCHED=1
systemctl daemon-reload
...
for attempt in $(seq 1 20); do
  ...
  if [[ "$after_pid" =~ ^[1-9][0-9]*$ && "$after_pid" != "$before_pid" && "$after_restarts" == "$before_restarts" &&
    ...
    "$(durable_runtime_revision)" == 137 && "$(readlink -f /opt/stay/current)" == "$STAY_R137_TARGET_RELEASE" && -S "$SOCKET" && ! -L "$SOCKET" ]] &&
    curl --fail --silent --max-time 1 http://127.0.0.1:8787/healthz | grep -q '"revision":137'; then
    ready=1; printf '%s\n' "$attempt" > "$WORK/readiness.attempts"; break
  fi
  sleep 0.25
done
[[ "$ready" -eq 1 ]] || abort r137-restart-readiness-failed 3315

capture_quiescent "$WORK/database.after.json" || abort database-after-not-quiescent 3316
/usr/local/bin/node "$STAY_R137_TARGET_RELEASE/$CLIENT" status resident:sntss > "$WORK/sntss.after.json"
/usr/local/bin/node "$STAY_R137_TARGET_RELEASE/$CLIENT" status resident:chronobiology > "$WORK/chronobiology.after.json"
/usr/local/bin/node "$STAY_R137_TARGET_RELEASE/$CLIENT" status resident:metab > "$WORK/metab.after.json"
curl --fail --silent --max-time 3 http://127.0.0.1:8787/__stay/meta > "$WORK/meta.after.json"
/usr/local/bin/node - "$WORK" "$STAY_R137_TARGET_RELEASE" "$before_pid" "$after_pid" "$before_restarts" "$after_restarts" > "$WORK/after.proof.json" <<'NODE'
...
const contained=v=>v?.status==='RUNNING'&&v.running===true&&v.authorityOwned===false&&v.host?.quarantined===false&&v.host?.osContainment?.required===true&&v.host?.osContainment?.available===true&&v.host?.osContainment?.payloadSandboxed===true&&v.host?.osContainment?.payloadAttachedBeforeInit===true&&v.host?.osContainment?.supervisorChargedToKernel===true&&v.host?.resourceGovernor?.policy?.softRamBytes===67108864&&v.host?.resourceGovernor?.policy?.hardRamBytes===100663296&&v.host?.resourceGovernor?.policy?.hardCpuDuty===0.2&&v.host?.resourceGovernor?.policy?.handlerTimeoutMs===250&&v.host?.resourceGovernor?.policy?.pidsMax===16&&v.host?.osContainment?.limits?.['memory.high']==='67108864'&&v.host?.osContainment?.limits?.['memory.max']==='100663296'&&v.host?.osContainment?.limits?.['pids.max']==='16'&&v.host?.osContainment?.limits?.['cpu.max']==='20000 100000';
if(!(before.result==='PASS'&&current===process.env.STAY_R137_TARGET_RELEASE&&Number(beforePid)>0&&Number(afterPid)>0&&beforePid!==afterPid&&beforeRestarts===afterRestarts&&
db.quickCheck==='ok'&&db.runtimeRevision===137&&db.pendingDeliveries===0&&db.pendingOutboxIntents===0&&db.failedDeliveries===0&&db.abandonedDeliveries===before.abandonedDeliveries&&db.p1Authority===0&&db.sntssAuthority===0&&db.chronobiologyAuthority===0&&db.metabOutboxIntents===0&&
...
state?.activation?.instanceId===before.metabInstanceId&&state?.activation?.runtimeRevision===137&&state?.activation?.sourceCheckpointHash===`sha256:${before.metabCheckpointHash}`&&state?.lastAcceptedFrame>=1&&state?.engineState?.outputSequence==='0'&&
source?.runtimeRevision===137&&source?.instanceId===before.metabInstanceId&&source?.residentVersion==='0.2.0-p1r0-shadow.1'&&source?.pending===null&&source?.lastCommittedFrame===state.lastAcceptedFrame&&
...
const release=Object.fromEntries(fs.readFileSync(path.join(root,'P1_R137_RELEASE.env'),'utf8').trim().split('\n').map(line=>{const at=line.indexOf('=');return[line.slice(0,at),line.slice(at+1)];}));
if(!(before.result==='PASS'&&after.result==='PASS'&&after.runtimeRevision===137&&after.instanceId===before.metabInstanceId))process.exit(2);
const names=['R127.freeze.json','R135.failure-marker.env','before.proof.json','after.proof.json','database.before.json','database.after.json','sntss.before.json','sntss.after.json','chronobiology.before.json','chronobiology.after.json','metab.before.json','metab.after.json','fetus.before.html','meta.after.json','P1_R137_RELEASE.env'];
const record=sealRevisionFreeze({format:'stay-runtime-revision-freeze-v1',result:'PASS',acceptance:'ACCEPTED',freezeType:'R137_METAB_OUTPUT_FIREWALLED_SHADOW_FORWARD_RECOVERY',runtime:{revision:137,revisionLabel:'R137F',progression:[123,124,125,126,127,128,129,130,131,132,133,134,135,136,137],serviceMainPid:Number(process.env.STAY_R137_AFTER_PID),serviceNRestarts:Number(process.env.STAY_R137_AFTER_RESTARTS),restartCommands:1},parentFreeze:{revision:127,recordSha256:parent.recordSha256},recoveryBoundary:{revision:135,r135FailureMarkerSha256:hash('R135.failure-marker.env'),pointerRewound:false},release:{path:release.RELEASE_PATH,tag:release.RELEASE_TAG,commit:release.RELEASE_COMMIT,tree:release.RELEASE_TREE,archiveSha256:release.ARCHIVE_SHA256,manifestSha256:release.MANIFEST_SHA256,controllerSha256:release.CONTROLLER_SHA256},metab:{residencyId:'resident:metab',instanceId:after.instanceId,version:after.version,mode:'SHADOW',checkpointGeneration:after.checkpointGeneration,acceptedFrame:after.acceptedFrame,authorityOwned:false,observedOutputs:0,outputPolicy:'FORBIDDEN_UNTIL_HOMEOS_ATTACHMENT'},continuity:{sntssCheckpointGenerationBefore:before.sntssCheckpointGeneration,chronobiologyCheckpointGenerationBefore:before.chronobiologyCheckpointGeneration,pendingDeliveries:0,pendingOutboxIntents:0,abandonedDeliveries:before.abandonedDeliveries,inventedBiologicalTime:false,fetusContinuity:true},recovery:{revisionFenced:true,pointerRewound:false},promotionAuthority:{startupOnly:true,revokedFromUnit:true},evidence:Object.fromEntries(names.map(n=>[n,hash(n)])),capturedAt:new Date().toISOString()});
if(!validateRevisionFreeze(record,137))process.exit(3);process.stdout.write(JSON.stringify(record)+'\n');
NODE
install_atomic "$WORK/R137.freeze.json" "$TARGET_FREEZE" 0444
rm -f -- "$SOURCE_MARKER"
rm -f -- "$R137_MARKER"
...
if(!(m.ok===true&&m.revision===137&&m.revisionFrozen===true&&m.revisionLabel==='R137F'&&chip('bsf')?.state==='LIVE'&&chip('sntss')?.state==='SHADOW'&&chip('chronobiology')?.state==='SHADOW'&&chip('metab')?.state==='SHADOW'))process.exit(1);
NODE
[[ "$(durable_runtime_revision)" == 137 && "$(readlink -f /opt/stay/current)" == "$STAY_R137_TARGET_RELEASE" &&
  "$(systemctl show stay.service -p MainPID --value)" == "$after_pid" &&
  "$(systemctl show stay.service -p NRestarts --value)" == "$after_restarts" ]] || abort final-r137-fence-failed 3318

final_evidence="$EVIDENCE_ROOT/R137F-METAB-SHADOW-$(date -u +'%Y%m%dT%H%M%SZ')"
[[ ! -e "$final_evidence" && ! -L "$final_evidence" ]]
mv -T "$WORK" "$final_evidence"; WORK=''; chmod -R a-w "$final_evidence"; COMPLETED=1
printf '%s\n' 'R137_METAB_SHADOW_RECOVERY=PASS' 'RUNTIME_REVISION=137' 'REVISION_LABEL=R137F' \
  "CURRENT_RELEASE=$STAY_R137_TARGET_RELEASE" "SERVICE_PID=$after_pid" 'BSF_MODE=LIVE' \
