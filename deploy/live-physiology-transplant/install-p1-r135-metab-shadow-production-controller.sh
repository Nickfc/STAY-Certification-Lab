...
EXPECTED_WRAPPER_SHA256='0cbc82d406e49447e39971dcabf04c7c7e10ce577949c58fe3ca5ab2d7c53f0c'
SCRIPT_DIR="$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)"
SOURCE_WRAPPER="$SCRIPT_DIR/stay-p1-r135-metab-shadow-production-controller"
TARGET_WRAPPER='/usr/local/sbin/stay-p1-production-controller'
TARGET_SUDOERS='/etc/sudoers.d/stay-p1-production-controller'

[[ "$EUID" -eq 0 ]] || {
  echo 'R135_BOOTSTRAP_ABORT=root-required' >&2
  exit 60
}
...
[[ "$observed_private_ipv4" == "$EXPECTED_PRIVATE_IPV4" ]] || {
  echo 'R135_BOOTSTRAP_ABORT=host-identity-mismatch' >&2
  exit 61
}
[[ -f "$SOURCE_WRAPPER" && ! -L "$SOURCE_WRAPPER" ]] || {
  echo 'R135_BOOTSTRAP_ABORT=wrapper-source-invalid' >&2
  exit 62
}
[[ "$(sha256sum "$SOURCE_WRAPPER" | awk '{print $1}')" == "$EXPECTED_WRAPPER_SHA256" ]] || {
  echo 'R135_BOOTSTRAP_ABORT=wrapper-source-hash-mismatch' >&2
  exit 63
}
command -v visudo >/dev/null 2>&1 || {
  echo 'R135_BOOTSTRAP_ABORT=visudo-unavailable' >&2
  exit 64
}
id staydeploy >/dev/null 2>&1 || {
  echo 'R135_BOOTSTRAP_ABORT=staydeploy-user-missing' >&2
  exit 65
}

staging="$(mktemp -d /run/stay-r135-metab-shadow-recovery-v1-bootstrap.XXXXXX)"
cleanup() {
  local status=$?
  trap - EXIT
  [[ "$staging" =~ ^/run/stay-r135-metab-shadow-recovery-v1-bootstrap\.[A-Za-z0-9]+$ &&
...
echo 'R135_FORWARD_AUTHORIZED=NO'
echo 'R135_RECOVERY_AUTHORIZED=NO'
