...
EXPECTED_WRAPPER_SHA256='cf66b744ee46275e740f55e48355ec8ee4aa6967b6b25ae91fdca8aa7d50d6ef'
SCRIPT_DIR="$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)"
SOURCE_WRAPPER="$SCRIPT_DIR/stay-p1-r137-metab-shadow-production-controller"
TARGET_WRAPPER='/usr/local/sbin/stay-p1-production-controller'
TARGET_SUDOERS='/etc/sudoers.d/stay-p1-production-controller'

[[ "$EUID" -eq 0 ]] || {
  echo 'R137_BOOTSTRAP_ABORT=root-required' >&2
  exit 60
}
...
[[ "$observed_private_ipv4" == "$EXPECTED_PRIVATE_IPV4" ]] || {
  echo 'R137_BOOTSTRAP_ABORT=host-identity-mismatch' >&2
  exit 61
}
[[ -f "$SOURCE_WRAPPER" && ! -L "$SOURCE_WRAPPER" ]] || {
  echo 'R137_BOOTSTRAP_ABORT=wrapper-source-invalid' >&2
  exit 62
}
[[ "$(sha256sum "$SOURCE_WRAPPER" | awk '{print $1}')" == "$EXPECTED_WRAPPER_SHA256" ]] || {
  echo 'R137_BOOTSTRAP_ABORT=wrapper-source-hash-mismatch' >&2
  exit 63
}
command -v visudo >/dev/null 2>&1 || {
  echo 'R137_BOOTSTRAP_ABORT=visudo-unavailable' >&2
  exit 64
}
id staydeploy >/dev/null 2>&1 || {
  echo 'R137_BOOTSTRAP_ABORT=staydeploy-user-missing' >&2
  exit 65
}

staging="$(mktemp -d /run/stay-r137-metab-shadow-recovery-v1-bootstrap.XXXXXX)"
cleanup() {
  local status=$?
  trap - EXIT
  [[ "$staging" =~ ^/run/stay-r137-metab-shadow-recovery-v1-bootstrap\.[A-Za-z0-9]+$ &&
...
echo 'R137_FORWARD_AUTHORIZED=NO'
echo 'R137_RECOVERY_AUTHORIZED=NO'
