...
contained(s)&&s?.instanceId==='8c65a965-5236-46e1-a2f1-e2f8cfc1ac0f'&&s?.version==='0.5.0-i4g1'&&s?.observedOutputs===0&&
contained(c)&&c?.instanceId==='f1e1ae54-9ea0-4d64-a9c6-6e4a301c5e8a'&&c?.version==='1.0.0-c3rc.5'&&c?.health?.mode==='NEUTRAL'&&
contained(m)&&m?.instanceId==='d424c722-ef31-44b0-8201-ba68c418d14a'&&m?.version==='0.2.0-p1r0-shadow.1'&&m?.health?.mode==='SHADOW'&&m?.declaredOutputs===0&&m?.observedOutputs===0&&m?.health?.outputPolicy==='FORBIDDEN_UNTIL_HOMEOS_ATTACHMENT'&&
